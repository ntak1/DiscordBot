...
export class Villagers {
  ...
  public marriageVillagers: string[] = [];
  public villagersNames: string[] = [];
  ...
  constructor() {
    ...
    this.initMarriageVillagers();
    this.initVillagersNames();
  }

  private initMarriageVillagers() {
    const marriageVillagers = [
      alex, elliott, harvey, sam, sebastian, shane,
      abigail, haley, leah, maru, penny, emily
    ];
    this.marriageVillagers = marriageVillagers.map((v) => v.name);
  }

  private initVillagersNames() {
    this.villagersNames = this.villagersList.map((v) => v.name);
  }
}
